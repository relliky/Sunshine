...
/* 	 
	...
	3. Setting time mode
		...
		viii)When the system is reset, the time is set to the default value, 0.0h.
 ...
 */
 	switch(UserInputState)
	{
		...
		{
		// For test propose
			LedDigitControl(1, LED_DIGIT_BLINK, 1			);
		...
		{
		// For test propose
			LedDigitControl(3, LED_DIGIT_BLINK, 1			);
			LedDigitControl(2, LED_DIGIT_OFF  , NOTHING 	);
			LedDigitControl(1, LED_DIGIT_OFF  , NOTHING 	);
			LedDigitControl(4, LED_DIGIT_OFF  , NOTHING 	);
		// End of test propose code					
			...
			if(LedBlinkTimer == UNDEFINED_BLINK_TIMER)
			{
			...
			}else
				{
					...
					if(LedBlinkTimer == LED_BLINK_TIME)
					{
						...
						if( (LedDigit1En == LED_DIGIT_ON) || (LedDigit2En == LED_DIGIT_ON) || (LedDigit3En == LED_DIGIT_ON) || (LedDigit4En == LED_DIGIT_ON) )
						{
