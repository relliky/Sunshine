...
/* --------------------------------------------------------------------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------------------------------------------------------------------*/
// Time parameter set for stepper motor for one movement in timer1, i.e. the value deciding how long does it take stepper motor 
...
void Timer_4_48ms_with_ID_timer0() interrupt 1
{
	...
	/* ------------------------------------------------------------------------------------------------------------------------------------------------
		Description: Low level driver to drive stepper motor moving forward/backward for amount of time, set by TimerSetForStepperLastingInOneMovementInTimer1
					 It will drive each phase to ON, and drive other phase to OFF every timer1 interrupt called, i.e. 5ms. 
					 1.	When StepperForwardStart contains the value of YES, the driver set StepperForwardStart to NO and start to drive motor forward for a period, set by TimerSetForStepperLastingInOneMovementInTimer1.
					 2. StepperForwardStart must not be set to YES again until the StepperForwardStop is set YES and then back to NO, which means the forward movement has completed.
					 3. Backward movement is similar to forward movement.
					 4. Since it is a low-level driver in a timer interrupt, and it has to handshake with high-level driver by using handshaking signals StepperForwardStart/StepperForwardStop for forward movements,StepperBackwardStart/StepperBackwardStop for backward movements,  See RunStepperForwardForTime, RunStepperBackwardForTime for detail of handshaking.
	  ------------------------------------------------------------------------------------------------------------------------------------------------*/
