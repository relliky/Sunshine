...
#define NORMAL_DISPLAY_STATE_TO_IDLE_STATE_TIME    30
...
unsigned char TemperatureMeasured	  = 22;
 ...
 */
 ...
 while(1)
 {
	...
	for(a=10;a>0;a--)
 	{   
  		TemperatureMeasured = (unsigned char) tmp();
  	}
	
	DisplayStaticTemperature(TemperatureMeasured);
	...
	// {
		...
		// Display5sCounter++;
		// if(Display5sCounter == COUNT_5S_FOR_1S_TIMER*3 + 1)
		// {
			// Display5sCounter = 1;
		// }
...
// /* 	 
	...
	// 2. Normal display mode
		...
		// iv)  when no any button is pressed for 30s, the user input state switches to the idle mode, LED display is off.
 ...
 // */
	...
	// {
		...
		// {
			...
			// }else
				// {
					// if(Display5sCounter <= COUNT_5S_FOR_1S_TIMER*2)
					// {
						// DisplayStaticTemperature(TemperatureMeasured);
					// }else
						// {
